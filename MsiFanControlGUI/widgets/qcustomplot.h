...
// for newer Qt versions we have to declare the enums/flags as metatypes inside the namespace using
// Q_ENUM_NS/Q_FLAG_NS: if you change anything here, don't forget to change it for older Qt versions
// below, too, and at the start of the namespace in the fake moc-run class
...
// for older Qt versions we have to declare the enums/flags as metatypes outside the namespace using
// Q_DECLARE_METATYPE: if you change anything here, don't forget to change it for newer Qt versions
// above, too, and at the start of the namespace in the fake moc-run class
...
void QCPDataContainer<DataType>::add(const QVector<DataType> &data, bool alreadySorted)
{
    ...
    if (alreadySorted && oldSize > 0
        && !qcpLessThanSortKey<DataType>(
          *constBegin(), *(data.constEnd() - 1))) // prepend if new data is sorted and keys are all
...
class QCP_LIB_DECL QCPAbstractPlottable : public QCPLayerable
{
  ...
  public:
    ...
    virtual double selectTest(const QPointF &pos, bool onlySelectable, QVariant *details = nullptr)
      const Q_DECL_OVERRIDE = 0; // actually introduced in QCPLayerable as non-pure, but we want to
  ...
  public:
    ...
    */
    ...
    selectTest(const QPointF &pos, bool onlySelectable,
               QVariant *details = 0) const; // actually introduced in QCPLayerable as non-pure, but
